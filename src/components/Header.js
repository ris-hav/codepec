...
const ViewIcon = styled(ViewArrayIcon)`
  &:hover {
    transform: scale(1.2);
  }
`;
...
const Header = () => {
  const { copy, setCopy, view, setView } = useContext(DataContext);
    ...
    <Container>
      <Tool>
        ...
        {copy ? (
          ...
          <h4>
            <em>Unleash Your Creative Code</em>
          </h4>
        )}

        <ViewIcon
          style={{
            // fontSize: view && `1.9rem`,
            transform: view && `rotate(90deg)`,
            border: view && "1px dotted #fff",
